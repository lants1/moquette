...
public class ProtocolProcessor {
	...
	private void republishStoredInSession(String clientID) {
		...
		for (PublishEvent pubEvt : publishedEvents) {
			sendPublish(pubEvt.getClientID(), pubEvt.getTopic(), pubEvt.getQos(), pubEvt.getMessage(), false,
					pubEvt.getMessageID());
	...
	public void processPublish(ServerChannel session, PublishMessage msg) {
		...
		// not every message should be parsed for a init message
		handleInitManagedZoneMessages(msg, topic, user);

		// TODO lants1
		if (m_authorizator.canWrite(topic, user, clientID) && hasManagedAutorisation(topic, user, clientID)) {
	...
	private boolean hasManagedAutorisation(final String topic, final String user, String clientID) {
		// scans retained messages to be published to the new subscription
		Collection<IMessagesStore.StoredMessage> messages = m_messagesStore.searchMatching(new IMatchingCondition() {
			public boolean match(String key) {
				return SubscriptionsStore.matchTopics(key, "$Managed/" + topic + "/" + user);
			}
		});
		
		Collection<IMessagesStore.StoredMessage> emptyCheck = m_messagesStore.searchMatching(new IMatchingCondition() {
			public boolean match(String key) {
				return SubscriptionsStore.matchTopics(key, "$Managed/" + topic + "/" + "#");
			}
		});
		...
		for (IMessagesStore.StoredMessage storedMsg : messages) {
			String permissionsStr = new String(storedMsg.getPayload().array());
			try{
				JSONObject permissions = new JSONObject(permissionsStr);
				String perms = permissions.getString("permissions");
	...
	private void handleInitManagedZoneMessages(PublishMessage msg, final String topic, String user) {
		if (topicIsNotManaged(topic) && msg.getRemainingLength() < 100000) {
			try {
				String potentialInitMsgStr = new String(msg.getPayload().array());
				JSONObject potentialInitMsg = new JSONObject(potentialInitMsgStr);
				String perms = potentialInitMsg.getString("permissions");
				if (perms.contains("rw")) {
					String manTopic = "$Managed/" + topic + "/" + user;
					m_messagesStore.storeRetained(manTopic, msg.getPayload(), QOSType.RESERVED);
				}
			} catch (JSONException e) {
				// easypeasy
			}
	...
	private boolean topicIsNotManaged(String topic) {
		// TODO Lan
		return true;
	}
	...
	private void executePublish(String clientID, PublishMessage msg) {
		...
		if (qos == AbstractMessage.QOSType.MOST_ONE) { // QoS0
			...
			// NB publish to subscribers for QoS 2 happen upon PUBREL from
			// publisher
		}
	...
	void forward2Subscribers(PublishEvent pubEvt) {
		...
		for (final Subscription sub : subscriptions.matches(topic)) {
			...
			LOG.debug("Broker republishing to client <{}> topic <{}> qos <{}>, active {}", sub.getClientId(),
					sub.getTopicFilter(), qos, sub.isActive());
			ByteBuffer message = origMessage.duplicate();
			if (qos == AbstractMessage.QOSType.MOST_ONE && sub.isActive()) {
				// QoS 0
				// forwardPublishQoS0(sub.getClientId(), topic, qos, message,
				// false);
	...
	protected void sendPublish(String clientId, String topic, AbstractMessage.QOSType qos, ByteBuffer message,
		...
		LOG.debug("sendPublish invoked clientId <{}> on topic <{}> QoS {} retained {} messageID {}", clientId, topic,
		...
		// TODO lants1 why read in case of publish and not subscribe case?
		if (!m_authorizator.canRead(topic, user, clientId) || !hasManagedAutorisation(topic, user, clientId)) {
	...
	private boolean subscribeSingleTopic(Subscription newSubscription, final String topic) {
		LOG.info("<{}> subscribed to topic <{}> with QoS {}", newSubscription.getClientId(), topic,
		...
		for (IMessagesStore.StoredMessage storedMsg : messages) {
			...
			sendPublish(newSubscription.getClientId(), storedMsg.getTopic(), storedMsg.getQos(), storedMsg.getPayload(),
					true, packetID);
		}
		return true;
	}
