...
import io.moquette.spi.impl.events.*;
...
public class ProtocolProcessor {
	...
	public void processConnect(ServerChannel session, ConnectMessage msg) {
		...
		if (msg.isUserFlag()) {
			...
			LOG.debug("check valid requested from broker");
			if (!m_authenticator.checkValid(msg.getUsername(), pwd, msg.getClientID())) {
				if (pwd != null) {
					LOG.debug("user:" + msg.getUsername() + " pw:" + pwd.toString() + " clientid:" + msg.getClientID()
	...
	public void processPublish(ServerChannel session, PublishMessage msg) {
		...
		if (m_authorizator.canWrite(topic, user, clientID, anonymous, msg.getPayload())) {
			...
			LOG.debug("topic:" + topic + " user:" + user + " clientId:" + clientID + " anonymous:" + anonymous
					+ " doesn't have write credentials");
		}
	}
