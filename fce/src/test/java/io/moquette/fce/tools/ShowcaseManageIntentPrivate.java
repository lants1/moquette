...
public class ShowcaseManageIntentPrivate extends Showcase{
	...
	public static void initializeInternalMqttClient() throws Exception {
		...
		client.publish(ManagedZone.INTENT.getTopicPrefix()+"/test2", inputJsonGlobal.getBytes(), Showcase.FIRE_AND_FORGET, true);
		client.publish(ManagedZone.INTENT.getTopicPrefix()+"/test2", inputJsonPrivate.getBytes(), Showcase.FIRE_AND_FORGET, true);
		client.disconnect();
		client.close();
	}
